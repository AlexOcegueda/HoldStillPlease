<script>
  ...
  const funeralPhotos = photos.filter(photo => photo.category === "funeral");
...
<section class="p-4">
  <h1 class="text-2xl">Funeral Photos</h1>
  <div class="grid grid-cols-5 gap-4">
    {#each funeralPhotos as photo}
      <img
        src={photo.url}
  ...
  {#if selectedPhoto}
    <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50">
      <div>
        <button on:click={() => (selectedPhoto = null)}>✕</button>
